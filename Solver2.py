...
class State:
	...
	def slide(self, dir):
		...
		if MODE == Mode.TILES:
			...
			if not edge_data.dest in PORTALS:
	...
	def is_dead_end(self):
		...
		if (eggs_left == 2):
			# Chjlly muss von einem Ei zum anderen kommen, oder anders rum
		...
		# Alle Kombinationen von Eiern kann ich unmöglich abprüfen,
		...
		# Bestimme ein drittes Ei, das möglichst weit von den anderen
		# entfernt liegt.
		...
		for (egg1, egg2, egg3) in [ (e1, e2, e3), 
			...
			for node in EGG_DESTS[egg1]:
				...
				# Hier, wenn Chilly von egg1 nach egg2 kommen kann,
				# kommt Chilly auch von egg2 nach egg3?
...
def construct_graph():
	...
	while unexplored:
		...
		# darauf an, dass Chilly auf diesen rutscht, und nicht von wo aus
		# bzw. wohin ab da.
		if node in EGGS:
			egg_dests[node] =  { node }
			egg_dests[node] =  { node }
...
def search_solution():
	...
	# Anscheinend gibt es bei einer guten Lösung 80 Rutsche auf 9 Eier.
	# Anfangs profitiert der Algo vielleicht von vielen Rutschen,
	# damit es einfacher wird, die Sackgassen zu finden. 
	# Später vielleicht von vielen gefundenen Eiern.
	def push_to_heap(state):
		eggs_found = len(EGGS) - len(state.eggs)
		slide_count = len(state.access_path)
		state_tuple = (state.chilly_loc, set(state.forbidden), set(state.eggs), state.access_path)
		estimate = abs(eggs_found - sqrt(slide_count))
		heappush(unexplored, (estimate, state_tuple))
		
	def pop_from_heap():
		(_, state_tuple) = heappop(unexplored)
		(chilly_loc, forbidden, eggs, access_path) = state_tuple
		return State(chilly_loc, set(forbidden), set(eggs), access_path)
	
	# Liste der noch zu bearbeitenden Zwischenstände, wird ggf. in Dateien 
	# ausgelagert, wenn sie zu voll wird
	unexplored = []
	heap_max = 6000000
	...
	# Diese beiden Dateien dienen als externen Puffer, wenn zu viele Zustände auflaufen,
	# um im Hauptspeicher gehalten zu werden. Es werden immer Zustände aus einer Datei
	# (fp_in) gelesen und neue Zustände in eine andere Datei (fp_out) geschrieben. 
	...
	# 'fp_out' getauscht und die (jetzige) Datei 'fp_out' gelöscht.
	# Sobald etwas Platz im Hauptspeicher ist, werden Elemente von fp_in dort hereingemischt.
	with open("G:\\Temp\\tempfile1.dat", mode='w+b') as fp1:
		with open("G:\\Temp\\tempfile2.dat", mode='w+b') as fp2:
			fp_in = fp1
			fp_out = fp2
			...
			while True: 
				...
				if ((batch_state_count >= 200000) or
					...
					print(f"# slides: {current_slides},", 
						  ...
						  f"b-states: {batch_state_count},",
						  f"b-pruned: {batch_pruned_count},",
